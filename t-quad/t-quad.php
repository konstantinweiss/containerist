...
class Container_TQuad extends Containerist {
	var $title = '';
	var $type = 'standard';
	var $style = 'http://containerist.org/container/t-quad/t-quad.css';
	var $content = '';
	...
	function output_html() {
		...
		foreach ($this->items as $item) {
			...
			$i = 0;
			foreach($item as $value) {
				$item_html = str_replace('$teaser_'.$keys[$i], $value, $item_html);
				$i++;
			}
